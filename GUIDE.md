# claude-code_persona — 자세한 가이드 (GUIDE)
...
A. 페르소나 텍스트(성격·규칙)만 있고 **비밀정보는 없습니다.** 그래도 개인 설정이라 저장소는 **비공개(private)**로 두는 걸 권장합니다(이미 비공개).
...
**Q. 끄고 싶으면요?**
A. 잠깐: 대화에서 `그냥 답해`. 완전히: `claude plugin disable persona-plugin@claude-code-persona` 후 재시작.
...
1. 설치·활성 확인:
   ...
   - `disabled` → `claude plugin enable persona-plugin@claude-code-persona`
2. 구성요소 인식 확인:
   ```bash
...
순서대로:
```bash
claude plugin marketplace update claude-code-persona
claude plugin uninstall persona-plugin@claude-code-persona
claude plugin install persona-plugin@claude-code-persona
```
...
- 그 후 다시 `git clone https://github.com/sodam-ai/claude-code_persona.git`
