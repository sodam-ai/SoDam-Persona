...
A. Claude Code의 플러그인 목록에 `persona-plugin`이 추가되고, 새 세션부터 AI에게 성격·규칙이 자동 주입됩니다. 다른 프로그램이나 파일은 건드리지 않습니다.
...
**Q. 끄고 싶으면요?**
A. 잠깐: 대화에서 `그냥 답해`. 완전히: `claude plugin disable persona-plugin@claude-code-persona-setting` 후 재시작.
...
1. 설치·활성 확인:
   ...
   - `disabled` → `claude plugin enable persona-plugin@claude-code-persona-setting`
2. 구성요소 인식 확인:
   ```bash
   claude plugin details persona-plugin
   ```
...
순서대로:
```bash
claude plugin marketplace update claude-code-persona-setting
claude plugin uninstall persona-plugin@claude-code-persona-setting
claude plugin install persona-plugin@claude-code-persona-setting
```
...
- 그 후 다시 `git clone https://github.com/sodam-ai/claude-code_persona-setting.git`
...
**페르소나 성격·규칙을 바꾸려면** → `persona-plugin/hooks/persona_core.md` 수정 (항상 켜지는 정본).
**트리거 단어를 추가하려면** → 같은 `persona_core.md`의 트리거 목록에 추가 (정본은 코어 한 곳).
**특정 전문가 상세를 바꾸려면** → `persona-plugin/skills/persona-(이름)/SKILL.md` 수정.
...
```bash
# 1) 플러그인 형식 검사
claude plugin validate (저장소경로)/persona-plugin
# 2) 설치·인식 확인
claude plugin list
claude plugin details persona-plugin   # Skills(5) + Hooks(2) 확인
```
